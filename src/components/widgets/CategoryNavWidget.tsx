...
const CategoryNavWidget: React.FC<CategoryNavWidgetProps> = ({ data }) => {
    ...
    // PRINCIPLE: Slow In / Slow Out (Spring Physics) - DISABLED IN ADMIN
    ...
    const handleCategoryClick = (cat: CategoryItem) => {
        ...
        // PRINCIPLE: Secondary Action (Haptic Visual Feedback)
    ...
    const renderButton = (cat: CategoryItem, index: number) => {
        const isActive = selectedCategory === cat.label;

        // PRINCIPLE: Follow Through (Staggered entrance) - DISABLED IN ADMIN
        const entryAnimation = isAdmin ? {} : {
        ...
        const whileTapGlassEffect = isAdmin ? { scale: 1 } : { scale: 0.92, rotate: -1 };
        const whileTapMinimalEffect = isAdmin ? { scale: 1 } : { scale: 0.95, x: -5 };
        const whileTapCardsEffect = isAdmin ? { scale: 1 } : { scale: 0.9, y: 10 };
        ...
        if (buttonStyle === 'GLASS') {
            return (
                <motion.button
                    ...
                    initial={isAdmin ? false : "initial"}
                    animate={isAdmin ? false : "animate"}
                    whileTap={whileTapGlassEffect}
                ...
                >
                    <div className="text-3xl md:text-5xl font-black uppercase tracking-tighter italic text-center text-white drop-shadow-md z-10 transition-transform group-active:scale-95">
                        {cat.label}
                    </div>
                    {isActive && (
                        <motion.div
                    ...
                    <div className="absolute inset-0 bg-white/0 group-active:bg-white/5 transition-colors pointer-events-none" />
                </motion.button>
            );
        }
        ...
        if (buttonStyle === 'MINIMAL') {
            return (
                <motion.button
                    ...
                    initial={isAdmin ? false : "initial"}
                    animate={isAdmin ? false : "animate"}
                    whileTap={whileTapMinimalEffect}
                    onClick={() => handleCategoryClick(cat)}
                ...
                >
                    ...
                    {isActive && <motion.div layoutId="dot" className="w-2 h-2 rounded-full" style={{ backgroundColor: accentColor }} />}
                </motion.button>
            );
        }
